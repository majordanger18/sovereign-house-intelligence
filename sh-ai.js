...
function renderAiResult(area,a,id){
  ...
  const cappedFlags=[...flags.filter(f=>f.type==="red").slice(0,3),...flags.filter(f=>f.type!=="red").slice(0,5)];
  const flagsHtml=cappedFlags.map(f=>`<div style="display:flex;align-items:start;gap:6px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.03)"><span style="font-size:12px;flex-shrink:0">${f.type==="red"?"🔴":"🟡"}</span><span style="font-size:11px;color:#e2e8f0">${esc(f.message)}</span></div>`).join("");
  ...
  area.innerHTML=`<div style="margin-top:10px;border-radius:16px;overflow:hidden;border:1px solid ${vc}33;animation:fadeUp .3s ease">
    ...
    ${cappedFlags.length>0?`<div style="padding:0 16px 16px"><div style="font-size:9px;color:#64748b;font-weight:700;letter-spacing:2px;margin-bottom:6px">DEAL FLAGS</div><div style="padding:10px;border-radius:10px;background:rgba(255,255,255,0.02)">${flagsHtml}</div></div>`:""}
