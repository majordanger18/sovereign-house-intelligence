...
let ctQuoteDocs=[],_sqrContactId=null;
...
async function openCtDetail(cid){
  ...
  if(c.mobile)h+=`<div class="ct-info-row"><span class="ct-info-l">Mobile</span><a href="tel:${esc(c.mobile)}" style="color:#60a5fa;text-decoration:none;font-weight:600">${esc(c.mobile)}</a></div>`;
...
function openCtForm(editId,prefill){
  ...
  h+=`<div class="fld"><label>MOBILE</label><input id="cfMobile" type="tel" class="cinput" value="${isEdit?esc(c.mobile||""):esc(pf.mobile||"")}" oninput="formatPhone(this)"/></div></div>`;
  h+=`<div class="fld"><label>EMAIL</label><input id="cfEmail" type="email" class="cinput" value="${isEdit?esc(c.email||""):esc(pf.email||"")}"/></div>`;
...
async function saveContact(editId){
  ...
  const payload={
    ...
    contact_type:gv("cfType")||"other",phone:gv("cfPhone")||null,mobile:gv("cfMobile")||null,email:gv("cfEmail")||null,
...
// ═══ SUPPLIER QUOTES (document storage) ═══

async function loadCtQuotes(cid){
  const el=document.getElementById("ctDetailQuotes");if(!el)return;
  try{
    const q=await sb("supplier_quotes?contact_id=eq."+cid+"&quote_document_url=not.is.null&order=created_at.desc");
    ctQuoteDocs=Array.isArray(q)?q:[];
  }catch(e){console.error("Load quotes failed:",e);ctQuoteDocs=[];}

  let h='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><div style="font-size:10px;color:#d4af37;font-weight:700;letter-spacing:2px">QUOTES ('+ctQuoteDocs.length+')</div><button onclick="openQuoteUpload(\''+cid+'\')" class="btn" style="padding:6px 12px;font-size:11px;background:rgba(212,175,55,0.08);border:1px solid rgba(212,175,55,0.25);color:#d4af37;font-weight:700;min-height:auto;border-radius:8px">📷 Upload Quote</button></div>';

  if(!ctQuoteDocs.length){
    h+='<div style="padding:16px;text-align:center;color:#475569;font-size:11px">No quotes yet. Upload a quote to start tracking.</div>';
    el.innerHTML=h;return;
  }

  const show=ctQuoteDocs.slice(0,3);
  show.forEach(function(q){
    const dt=q.created_at?new Date(q.created_at).toLocaleDateString("en-US",{month:"numeric",day:"numeric",year:"2-digit",timeZone:"America/Los_Angeles"}):"";
    const fname=q.item_description||q.notes||"Quote document";
    h+='<a href="'+esc(q.quote_document_url)+'" target="_blank" style="display:flex;align-items:center;gap:10px;padding:10px;border-radius:10px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);margin-bottom:6px;text-decoration:none;transition:background .1s">';
    h+='<span style="font-size:20px;flex-shrink:0">📄</span>';
    h+='<div style="flex:1;min-width:0"><div style="font-size:12px;font-weight:600;color:#e2e8f0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(fname)+'</div><div style="font-size:10px;color:#64748b;margin-top:2px">'+dt+'</div></div>';
    h+='<span style="font-size:10px;color:#60a5fa;font-weight:700;flex-shrink:0">View</span>';
    h+='</a>';
  });

  if(ctQuoteDocs.length>3){
    h+='<button onclick="showAllCtQuotes(\''+cid+'\')" style="background:none;border:none;color:#60a5fa;font-size:11px;font-weight:700;cursor:pointer;padding:4px 0">View all '+ctQuoteDocs.length+' quotes</button>';
  }

  el.innerHTML=h;
}

function showAllCtQuotes(cid){
  const el=document.getElementById("ctDetailQuotes");if(!el)return;
  let h='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><div style="font-size:10px;color:#d4af37;font-weight:700;letter-spacing:2px">QUOTES ('+ctQuoteDocs.length+')</div><button onclick="openQuoteUpload(\''+cid+'\')" class="btn" style="padding:6px 12px;font-size:11px;background:rgba(212,175,55,0.08);border:1px solid rgba(212,175,55,0.25);color:#d4af37;font-weight:700;min-height:auto;border-radius:8px">📷 Upload Quote</button></div>';
  ctQuoteDocs.forEach(function(q){
    const dt=q.created_at?new Date(q.created_at).toLocaleDateString("en-US",{month:"numeric",day:"numeric",year:"2-digit",timeZone:"America/Los_Angeles"}):"";
    const fname=q.item_description||q.notes||"Quote document";
    h+='<a href="'+esc(q.quote_document_url)+'" target="_blank" style="display:flex;align-items:center;gap:10px;padding:10px;border-radius:10px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);margin-bottom:6px;text-decoration:none">';
    h+='<span style="font-size:20px;flex-shrink:0">📄</span>';
    h+='<div style="flex:1;min-width:0"><div style="font-size:12px;font-weight:600;color:#e2e8f0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(fname)+'</div><div style="font-size:10px;color:#64748b;margin-top:2px">'+dt+'</div></div>';
    h+='<span style="font-size:10px;color:#60a5fa;font-weight:700;flex-shrink:0">View</span>';
    h+='</a>';
...
function openQuoteUpload(contactId){
  ...
  input.onchange=function(){
    ...
    setTimeout(function(){uploadQuoteDoc(window._pendingQuoteFile);},100);
  };
  input.click();
}

async function uploadQuoteDoc(file){
  showCtToast("Uploading quote...");
  try{
    const path="contacts/"+_sqrContactId+"/quotes/"+Date.now()+"."+(file.name.split(".").pop()||"jpg");
    const url=await uploadToStorage(file,"sovereign-docs",path);
    if(!url){showCtToast("Upload failed");return;}

    const payload={contact_id:_sqrContactId,item_description:file.name.replace(/\.[^.]+$/,""),quote_document_url:url,status:"quoted",created_by:SH_USER?.email||"unknown"};
    await fetch(SB+"/rest/v1/supplier_quotes",{method:"POST",headers:HD,body:JSON.stringify(payload)});

    showCtToast("Quote saved");
    loadCtQuotes(_sqrContactId);
  }catch(e){
