...
function renderQuoteGroups(groups,keys){
  let h='';
  keys.forEach(function(date){
    ...
    const gid='sqg_'+date.replace(/\W/g,'');
    const dtLabel=date==="undated"?"No date":new Date(date+"T00:00:00").toLocaleDateString("en-US",{timeZone:"America/Los_Angeles"});
    ...
    h+='<div id="'+gid+'" style="display:none;padding:4px 0 8px 20px">';
...
async function parseQuote(file){
  ...
  try{
    if(file.type==='application/pdf'){
      ...
      if(w>maxDim||hh>maxDim){
        if(w>hh){hh=Math.round(hh*(maxDim/w));w=maxDim;}
        else{w=Math.round(w*(maxDim/hh));hh=maxDim;}
      }
      const canvas=document.createElement('canvas');
