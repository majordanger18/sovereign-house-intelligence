...
async function loadData(){
  try{
    const[p,w,a,ua,d,nm]=await Promise.all([sb("properties?status=neq.Closed&listing_status=neq.Gone&order=sovereign_score.desc"),sb("watchlist?status=eq.Watching"),sb("alerts?is_read=eq.false&order=created_at.desc&limit=50"),sb("underwriting_analyses?select=property_id,verdict,profit_target,roi_target,max_offer_price,max_offer_raw,max_offer_tier,max_offer_flag,max_offer_discount_pct,list_price,created_at&order=created_at.desc"),sb("deals?order=updated_at.desc"),sb("community_name_map?select=subdivision_name,community_name")]);
    ...
    (Array.isArray(nm)?nm:[]).forEach(x=>{if(x.subdivision_name)window.communityNameMap[x.subdivision_name]=x.community_name;});
...
const baseFilter=p=>p.disposition!=="passed"&&p.disposition!=="pursuing"&&p.disposition!=="acquired"&&p.listing_status!=="Closed"&&p.listing_status!=="Gone"&&!p.gone_date;

function renderDashboard(){
  const base=props.filter(baseFilter);
  const st={total:base.length,
    watched:base.filter(p=>watchIds.has(p.id)).length,
...
function renderList(){
  ...
  function cardHtml(p,i,extraStyle){
    ...
    return`<div class="card${isW?" watched":""}" onclick="openDetail('${p.id}')" style="animation:fadeUp .3s ease ${i*30}ms both;${extraStyle||""}${passedDim?"opacity:0.5;":isPending?"opacity:0.6":""}"><div style="display:flex;gap:12px;align-items:center">${ring(p.sovereign_score)}<div style="flex:1;min-width:0"><div style="display:flex;justify-content:space-between;gap:6px"><div style="min-width:0;flex:1"><div style="font-size:14px;font-weight:700;color:#f1f5f9;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${isW?'<span style="color:#d4af37">★ </span>':""}${esc(p.address)}</div><div style="font-size:10px;color:#64748b;margin-top:1px">${(p.subdivision_name&&truncSub((window.communityNameMap&&window.communityNameMap[p.subdivision_name])||p.subdivision_name))?`<span style="color:#94a3b8">${esc(truncSub((window.communityNameMap&&window.communityNameMap[p.subdivision_name])||p.subdivision_name))}</span> · `:""}${p.zip_code}</div>${passInfo}</div><div style="text-align:right;flex-shrink:0"><div style="font-size:17px;font-weight:800">${$(p.list_price)}</div>${p.price_per_sqft?`<div style="font-size:10px;color:#64748b">$${p.price_per_sqft}/sf</div>`:""}${isPassed?`<span class="passed-badge" style="margin-top:2px">PASSED</span>`:""}</div></div><div style="font-size:10px;color:#64748b;margin-top:4px">${stats}</div><div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px">${flags.join("")}</div>${aiRow}${pendingBadge}</div></div></div>`;
  }
