...
async function openDeal(dealId){
  ...
  m.innerHTML=`<div class="sheet" style="position:relative;max-height:90vh;overflow-y:auto"><div class="handle"></div><button class="close-x" onclick="closeDeal()">✕</button>
    ...
    <div style="margin-bottom:16px;padding:14px;border-radius:14px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06)">
      ...
      <div class="row2">
        <div class="fld"><label>NAME</label><div style="display:flex;align-items:center;gap:0"><input id="deal_ba_name" class="cinput" style="flex:1" value="${esc(d.buyer_agent_name||'Lisa Hunt')}" onchange="saveDealField('${d.id}','buyer_agent_name',this.value)"/><button type="button" class="cp-pick-btn" onclick="window.openContactPicker({title:'Select Buyer Agent',filterType:'agent',onSelect:function(c){var n=document.getElementById('deal_ba_name');n.value=c.display_name;saveDealField('${d.id}','buyer_agent_name',c.display_name);if(c.phone){var ph=document.getElementById('deal_ba_phone');ph.value=c.phone;saveDealField('${d.id}','buyer_agent_phone',c.phone);}if(c.email){var em=document.getElementById('deal_ba_email');em.value=c.email;saveDealField('${d.id}','buyer_agent_email',c.email);}},anchor:this})" title="Pick from contacts">&#128100;</button></div></div>
        <div class="fld"><label>PHONE</label><input id="deal_ba_phone" type="tel" class="cinput" value="${esc(d.buyer_agent_phone||'(702) 285-7491')}" onchange="saveDealField('${d.id}','buyer_agent_phone',this.value)"/></div>
      </div>
      <div class="fld"><label>EMAIL</label><input id="deal_ba_email" type="email" class="cinput" value="${esc(d.buyer_agent_email||'lisa@lisaahunt.com')}" onchange="saveDealField('${d.id}','buyer_agent_email',this.value)"/></div>
    </div>
    `:''}
